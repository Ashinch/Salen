/*
* Revised in https://github.com/Christop406/ical-parser
*/
  ...
  for (let i = 1; i < lines.length && (max === undefined || event_count < max); i++) {
    ...
    if (lines[i].indexOf('BEGIN:VEVENT') !== -1) {
      ...
      switch (key) {
        ...
        case 'LAST-MODIFIED':
        case 'DTSTAMP':
          events[event_count][k] = getDate(value)
          break
        case 'DTSTART':
        case 'DTEND':
          ...
          if (params['VALUE'] === 'DATE') {
            ...
            events[event_count][k].value = getDate(value)
          }
...
function getDate(value) {
  return value.indexOf('Z') === value.length - 1
    ? new Date(`${value.substring(0, 4)}/${value.substring(4, 6)}/${value.substring(6, 8)} ${value.substring(9, 11)}:${value.substring(11, 13)}:${value.substring(13, value.length)}`)
    : new Date(value.substring(0, 4), parseInt(value.substring(4, 6)) - 1, value.substring(6, 8), value.substring(9, 11), value.substring(11, 13), value.substring(13, 15))
}
